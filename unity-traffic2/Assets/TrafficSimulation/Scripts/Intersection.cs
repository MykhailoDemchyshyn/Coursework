...
{
    public enum IntersectionType //Тип перехрестя
    {
        STOP,
        TRAFFIC_LIGHT
    }
    ...
    public class Intersection : MonoBehaviour
    {
        ...
        void SwitchLights()
        {
            ...
            //Wait few seconds after light transition before making the other car move (= orange light) //Зачекайте кілька секунд після переходу світла перед тим, як змусити інший автомобіль рухатися (= оранжеве світло)
            Invoke("MoveVehiclesQueue", orangeLightDuration);
        }

        void OnTriggerEnter(Collider _other)
        {
            //Check if vehicle is already in the list if yes abort //Перевірте, чи транспортний засіб вже є у списку, якщо так, скасувати
            //Also abort if we just started the scene (if vehicles inside colliders at start) //Також перервати, якщо ми тільки розпочали сцену (якщо транспортні засоби, що знаходяться всередині колії на початку)
        ...
        void TriggerStop(GameObject _vehicle)
        {
            ...
            //Depending on the waypoint threshold, the car can be either on the target segment or on the past segment //Залежно від порогу маршрутної точки автомобіль може знаходитись як на цільовому сегменті, так і на минулому сегменті
